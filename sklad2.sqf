...
CONCAT(zlt_sp_obj,FNUM) = Sklad2;

// 2 параметр - маркера на точки установки
CONCAT(zlt_sp_allowedmrkrs,FNUM) = ["marker_31","marker_33", "marker_35"];
...
CONCAT(zlt_sp_selmrkrs,FNUM) = ["marker_30","marker_32", "marker_34"];

// 3 параметр - маркер на технику
CONCAT(zlt_sp_mrkend,FNUM) =  "marker_sklad2";
...
CONCAT(zlt_spActionId,FNUM) = player addAction ["<t color='#ff0000'>Выбрать место для склада 2</t>", {call CONCAT(zlt_sp_placeObj,FNUM)}];
//
[] spawn {
